...
namespace MorseCodeEffect.Effect
{
...

    {
        ...
        bool isAppend = false;
        [Display(Name = "テキスト", Description = "モールス信号に変換するテキスト(漢字は未対応です)")]
        [TextEditor(AcceptsReturn = true)]
        public string Text { get => text; set => Set(ref text, value); }
        string text = string.Empty;

        [Display(Name = "健打速度", Description = "生成するモールス信号の速度を調整します。")]
        ...
        [Display(Name = "音量", Description = "生成するモールス信号の音量を調節します")]
        [AnimationSlider("F2", "", 0, 10)]
        public Animation Volume { get; } = new Animation(1, 0, 100);

        [Display(Name = "ループ", Description = "モールス信号をループ")]
        [ToggleSlider]
        public bool IsLoop { get => isLoop; set => Set(ref isLoop, value); }
        bool isLoop = false;

        [Display(Name = "周波数", Description = "トンツーの音の高さを変更します")]
